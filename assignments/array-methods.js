...
let fullName = [];
runners.forEach(function(nameadd) {
    let name = `${nameadd.first_name} ${nameadd.last_name}`;
    return fullName.push(name);
})
...
let allCaps = runners.map(capital => {
    return capital.first_name.toUpperCase();
})
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((initial, runner) => {
